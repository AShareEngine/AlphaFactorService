...
单模型LightGBM、XGBoost和CatBoost支持冻结`optuna`配置后执行10至100次TPE搜索，唯一目标为
验证段Rank ICIR最大化。搜索trial与最佳参数写入模型Manifest和`optuna_trials.json`，测试段不参与
选择；最佳参数随后用于正式训练。验证长度为0、增量续训、参数实验、多模型对比和Stacking不允许
